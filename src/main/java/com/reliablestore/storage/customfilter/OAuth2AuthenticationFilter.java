package com.reliablestore.storage.customfilter;
...
public class OAuth2AuthenticationFilter extends GenericFilter {
    ...
    private boolean hasCognitoAdminRole(Authentication authentication) {
        if (((OAuth2User)authentication.getPrincipal()).getAttribute("cognito:groups") != null) {
            String test=((OAuth2User)authentication.getPrincipal()).getAttribute("cognito:groups").toString();
            return ((OAuth2User)authentication.getPrincipal()).getAttribute("cognito:groups").toString().contains("admin");
        } else {
            return false;
        }
    }
